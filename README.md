...
## Исправления для Яндекс Директа

### Валюта

В `currencyId` всегда используется `RUB`, потому что Яндекс Директ не принимает внутреннюю валюту магазина как полноценную валюту фида. Если книга продаётся за сапфиры, цена остаётся в `<price>`, но дополнительно в оффере появляются параметры:

```xml
<param name="internal_currency">sapphire</param>
<param name="sapphires_price">199</param>
```

### Исключение товаров “N сапфиров - RC Books”

Пакеты сапфиров не являются книгами и исключаются по регулярным выражениям в `input/config.yml`:
